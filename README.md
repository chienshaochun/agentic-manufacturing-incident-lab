...
Phase 15 已完成：事件調查台只顯示症狀與中性模擬批次，不再把根因放在輸入選單。UI 會逐筆呈現 Observation 如何改變 Hypothesis，列出 Planner 所有候選 Tool 的 Utility 拆解；Raw Trace 同時保存 Observation 品質、支持與反對關係及候選決策。完整 benchmark 仍維持 16 個受控案例。
